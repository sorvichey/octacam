                ...
                <div class="infinite-scroll">
                    @foreach($posts as $p)
                        <div class="single-blog-post small-featured-post shadow-sm p-3  bg-light rounded">
                            <div class="post-data">
                                <a href="{{asset('post/detail/'.$p->id)}}" class="post-catagory">{{str_limit(strip_tags($p->title), $p->title = 210, $end = '...')}}</a>
                            </div>
                            <hr>
                        <div class="row">
                            @if($p->featured_image != null)
                            <div class="col-md-4 featured-image">
                                <a href="{{asset('post/detail/'.$p->id)}}"><img src="{{asset('uploads/posts/small/'.$p->featured_image)}}" width="100%" alt=""></a>
                            </div>
                            @endif
                            @if($p->featured_image != null)
                            ...
                            @endif
                                <a href="{{asset('post/detail/'.$p->id)}}" class="post-title">
